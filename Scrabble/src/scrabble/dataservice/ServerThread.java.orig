...
public class ServerThread extends Thread {
    ...
    public void controlInPlay () throws IOException
    {
        ...
        for (int i = 0; i < clientSocketList.size(); i++)
        {
            System.out.println (" in for number" + i);
            System.out.println (" in for");
            Socket dstSocket;
            dstSocket = clientSocketList.elementAt(i);
            try{
                DataOutputStream out = new DataOutputStream(dstSocket.getOutputStream());
                Vector <Tile> tiles = game.getTiles(game.getPlayerList().elementAt(clientSocketList.indexOf(skt)).getUsername());
